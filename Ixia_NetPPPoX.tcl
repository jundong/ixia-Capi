...
class PppoeHost {
    ...
    method CreatePPPoEPerSessionView {} {
        set tag "body DhcpHost::CreatePPPoEPerSessionView [info script]"
Deputs "----- TAG: $tag -----"
...
body PppoeHost::reborn { { onStack null } } {
	...
	set sg_pppoxEndpoint [ixNet getL $sg_ethernet pppoxEndpoint]
	if {$sg_pppoxEndpoint == "" && $onStack == "null"} {
...
body PppoeHost::config { args } {
    ...
    set EAuth      [ list none auto chap_md5 pap papsender chapresponder ]
...
body PppoeHost::disable {} {
    set tag "body PppoeHost::disable [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet setA $handle/pppoxRange \
		 -enabled false
...
body PppoeHost::enable {} {
    set tag "body PppoeHost::enable [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
...
body PppoeHost::disconnect {} {
    set tag "body PppoeHost::disconnect [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet exec pppoxStop $handle async
  
}

body PppoeHost::retry {} {
    set tag "body PppoeHost::retry [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet exec pppoxRetry $handle async
  
}

body PppoeHost::pause {} {
    set tag "body PppoeHost::pause [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet exec pppoxPause $handle async
  
}

body PppoeHost::resume {} {
    set tag "body PppoeHost::resume [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet exec pppoxResume $handle async
  
}

body PppoeHost::cancel {} {
    set tag "body PppoeHost::cancel [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
...
body PppoeHost::abort {} {
    set tag "body PppoeHost::abort [info script]"
Deputs "----- TAG: $tag -----"
Deputs "handle :$handle"
		
    ixNet exec abort $hPppox async
...
class Pppoev6Host {
    ...
    constructor { port { onStack null } } { chain $port $onStack } {}
	method reborn { { onStack null } } {
    set tag "body Pppoev6Host::reborn [info script]"
	Deputs "----- TAG: $tag -----"
	    chain
		config -ipcp_encap "ipv6"
...
class Pppoev4v6Host {
    ...
    constructor { port { onStack null } } { chain $port $onStack } {}
	method reborn { { onStack null } } {
    set tag "body Pppoev4v6Host::reborn [info script]"
	Deputs "----- TAG: $tag -----"
	    chain
		config -ipcp_encap "ipv4v6"
...
class Pppoev6Server {
    ...
    constructor { port { onStack null } } { chain $port $onStack } {}
	method reborn { { onStack null } } {
    set tag "body Pppoev6Server::reborn [info script]"
	Deputs "----- TAG: $tag -----"
	    chain
        set optionSet [ixNet getL $hPort/protocolStack "pppoxOptions"]
        if {$optionSet == "" } {
...
class Pppoev4v6Server {
    ...
    constructor { port { onStack null } } { chain $port $onStack } {}
	method reborn { { onStack null } } {
    set tag "body Pppoev4v6Server::reborn [info script]"
	Deputs "----- TAG: $tag -----"
	    chain
        set optionSet [ixNet getL $hPort/protocolStack "pppoxOptions"]
        if {$optionSet == "" } {
