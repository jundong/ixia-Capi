...
Deputs "ixNet setA $handle/dhcpRange -clientOptionSet $optionSet"
    ...
    method rebind {} {}
	method enable {} {
	    set tag "body DhcpHost::enable [info script]"
	    Deputs "----- TAG: $tag -----"
		
	    ixNet setA $handle/dhcpRange  -enabled True
	    ixNet commit
	
	}
	method disable {} {
	    set tag "body DhcpHost::disable [info script]"
	    Deputs "----- TAG: $tag -----"
		
	    ixNet setA $handle/dhcpRange  -enabled False
	    ixNet commit
		   ...
		   -iaid {
			...
			-ia_type {
                ...
                if { [ lsearch -exact $ESession $value ] >= 0 } {
				    ...
				    set session_type  iana
				} elseif { $value == "pd"} {
				    set session_type  iapd
				} elseif { $value == "ipv6pd"} {
				    set session_type  "iana+iapd"
				} else {
                    error "$errNumber(1) key:$key value:$value"
                }                
			}
...
class Dot1xHost {
    ...
    public variable type
	public variable sg_ethernet
	public variable hDot1x
...
Deputs "----- TAG: $tag -----"
        ixNet exec dot1xStop $hDot1x
	
	    ixNet commit
        return [GetStandardReturnHeader]
    }
	method abort {} {
     set tag "body dot1xHost::abort [info script]"
Deputs "----- TAG: $tag -----"
        ixNet exec abort $sg_ethernet
...
Deputs "Args:$args "
    foreach { key value } $args {
        ...
        switch -exact -- $key {
            ...
            -auth_type {
				set authtype [string toupper $value]
            }
            -user_name {
				set username  $value
            }
	...
	ixNet setA $dot1x_range -name $this
	ixNet commit
	...
	ixNet setA $dot1x_range -enabled $enabled
	ixNet commit

}

body Dot1xHost::get_detailed_stats {} {
    global errorInfo
    global errNumber
    set tag "body Dot1xHost::get_detailed_stats [info script]"
Deputs "----- TAG: $tag -----"
    set root [ixNet getRoot]
		
		set view [lindex [ ixNet getF $root/statistics view -caption "802.1x" ] 0]
		...
		set success_index       [ lsearch -glob $captionList *Succeeded ]
		set failed_index        [ lsearch -glob $captionList *Failed ]
		set stats [ ixNet getA $view/page -rowValues ]
